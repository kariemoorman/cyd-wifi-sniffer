...
def load_alerts(data_dir: str) -> dict:
    ...
    if alerts:
        print(
            f"  Loaded alerts for {len(alerts)} devices from {len(files)} alert files"
        )
    return alerts
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate training labels from sniffer data"
    )
    parser.add_argument(
        "--data_dir",
        required=True,
        help="Directory with feat_*.csv and alert_*.csv files",
    )
